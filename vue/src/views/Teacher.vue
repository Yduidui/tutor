  ...
  <div class="teacher-container">
    ...
    <el-card class="action-card" shadow="hover">
      <div class="action-wrapper">
        ...
        <el-upload
            class="upload-btn"
            action="http://localhost:8080/teacher/import"
    ...
    <el-card class="table-card" shadow="hover">
      ...
      >
        ...
        <el-table-column label="教授科目" prop="subject" min-width="120" />
        <el-table-column label="教学课程总数" prop="totalClasses" width="120" align="center" />
        <el-table-column label="教授科目数" prop="totalSubjects" width="120" align="center" />
        <el-table-column label="累计授课小时" prop="totalHours" width="120" align="center" />
    ...
    <el-drawer
        ...
        :title="data.form.id ? '编辑教师信息' : '新增教师'"
        size="600px"
        destroy-on-close
    >
      <el-form
      ...
      >
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="教授科目" prop="subject">
              <el-input v-model="data.form.subject" placeholder="请输入教授科目" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          <el-col>
            <el-form-item label="评分" prop="rating">
              <el-rate
                  v-model="data.form.rating"
                  disabled
                  show-score
                  text-color="#ff9900"
                  :score-template="`${data.form.rating} 分`"
              />
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="资质证明" prop="qualificationPhotos">
              <el-upload
              ...
              >
                <img
                    v-if="data.form.qualificationPhotos"
                    :src="data.form.qualificationPhotos"
                    class="qualification"
                />
                <el-icon v-else class="qualification-uploader-icon"><Plus /></el-icon>
              </el-upload>
            </el-form-item>
          </el-col>
        </el-row>
...
// 导出数据
const exportData = () => {
  window.open('http://localhost:8080/teacher/export');
};
...
// 资质证明上传成功
const handleQualificationSuccess = (res) => {
  data.form.qualificationPhotos = res.data;
};
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除数据后无法恢复', '删除确认', { type: 'warning' }).then(() => {
    request.delete('/teacher/deleteBatch', { data: data.ids }).then((res) => {
      if (res.code === '200') {
        ElMessage.success('操作成功');
        load();
      } else {
...
.qualification-uploader .el-upload {
...
.qualification-uploader .el-upload:hover {
  border-color: var(--el-color-primary);
}

.el-icon.avatar-uploader-icon,
.el-icon.qualification-uploader-icon {
  font-size: 28px;
  color: #8c939d;
  width: 100px;
  height: 100px;
